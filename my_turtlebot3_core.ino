...
void setup()
{
  ...
  nh.advertise(CRS03_pub);
  nh.advertise(CRS09_pub);
...
void loop()
{
  ...
  // Start Analog Gyro Calibration after ROS connection
  updateCRSCali(nh.connected());
...
/*******************************************************************************
  Publish msgs (analog gyro data: angular velocity)
*******************************************************************************/
void publishCRS03Msg(void)
{
  CRS03_msg = sensors.getIMU();

  CRS03_msg.header.stamp    = rosNow();
  CRS03_msg.header.frame_id = imu_frame_id;

  float angular_vel = -DEG2RAD((analogRead(CRS03_PIN) - CRS03_zero) * ANALOG_CONV / CRSO3_FACTOR);
  //angular_vel = lpfilter(angular_vel);

  if (abs(angular_vel) < 0.00319260009564)
    angular_vel = 0;

  CRS03_msg.orientation.x = 0;
  CRS03_msg.orientation.y = 0;
  CRS03_msg.orientation.z = 0;
  CRS03_msg.orientation.w = 0;

  CRS03_msg.orientation_covariance[0] = 0;
  CRS03_msg.orientation_covariance[1] = 0;
  CRS03_msg.orientation_covariance[2] = 0;
  CRS03_msg.orientation_covariance[3] = 0;
  CRS03_msg.orientation_covariance[4] = 0;
  CRS03_msg.orientation_covariance[5] = 0;
  CRS03_msg.orientation_covariance[6] = 0;
  CRS03_msg.orientation_covariance[7] = 0;
  CRS03_msg.orientation_covariance[8] = 0;

  CRS03_msg.angular_velocity.x = 0;
  CRS03_msg.angular_velocity.y = 0;
  CRS03_msg.angular_velocity.z = angular_vel;

  CRS03_msg.angular_velocity_covariance[0] = 0;
  CRS03_msg.angular_velocity_covariance[1] = 0;
  CRS03_msg.angular_velocity_covariance[2] = 0;
  CRS03_msg.angular_velocity_covariance[3] = 0;
  CRS03_msg.angular_velocity_covariance[4] = 0;
  CRS03_msg.angular_velocity_covariance[5] = 0;
  CRS03_msg.angular_velocity_covariance[6] = 0;
  CRS03_msg.angular_velocity_covariance[7] = 0;
  CRS03_msg.angular_velocity_covariance[8] = 0.02;

  CRS03_pub.publish(&CRS03_msg);
}

/*******************************************************************************
  Publish msgs (analog gyro data: angular velocity)
*******************************************************************************/
void publishCRS09Msg(void)
{
  CRS09_msg = sensors.getIMU();

  CRS09_msg.header.stamp    = rosNow();
  CRS09_msg.header.frame_id = imu_frame_id;

  float angular_vel = -DEG2RAD((analogRead(CRS09_PIN) - CRS09_zero) * ANALOG_CONV / CRSO9_FACTOR);
  //angular_vel = lpfilter(angular_vel);
  
  if (abs(angular_vel) < 0.00319260009564)
    angular_vel = 0;

  CRS09_msg.orientation.x = 0;
  CRS09_msg.orientation.y = 0;
  CRS09_msg.orientation.z = 0;
  CRS09_msg.orientation.w = 0;

  CRS09_msg.orientation_covariance[0] = 0;
  CRS09_msg.orientation_covariance[1] = 0;
  CRS09_msg.orientation_covariance[2] = 0;
  CRS09_msg.orientation_covariance[3] = 0;
  CRS09_msg.orientation_covariance[4] = 0;
  CRS09_msg.orientation_covariance[5] = 0;
  CRS09_msg.orientation_covariance[6] = 0;
  CRS09_msg.orientation_covariance[7] = 0;
  CRS09_msg.orientation_covariance[8] = 0;

  CRS09_msg.angular_velocity.x = 0;
  CRS09_msg.angular_velocity.y = 0;
  CRS09_msg.angular_velocity.z = angular_vel;

  CRS09_msg.angular_velocity_covariance[0] = 0;
  CRS09_msg.angular_velocity_covariance[1] = 0;
  CRS09_msg.angular_velocity_covariance[2] = 0;
  CRS09_msg.angular_velocity_covariance[3] = 0;
  CRS09_msg.angular_velocity_covariance[4] = 0;
  CRS09_msg.angular_velocity_covariance[5] = 0;
  CRS09_msg.angular_velocity_covariance[6] = 0;
  CRS09_msg.angular_velocity_covariance[7] = 0;
  CRS09_msg.angular_velocity_covariance[8] = 0.02;

  CRS09_pub.publish(&CRS09_msg);
}
...
float lpfilter(float input)
{
  ...
  yv[6] =   (xv[0] + xv[6]) + 6 * (xv[1] + xv[5]) + 15 * (xv[2] + xv[4])
            + 20 * xv[3]
            + ( -0.2270691521 * yv[0]) + (  1.0641317688 * yv[1])
            + ( -2.5372712245 * yv[2]) + (  3.8150972853 * yv[3])
            + ( -3.8747177893 * yv[4]) + (  2.5047849952 * yv[5]);
  return yv[6];
}
...
*******************************************************************************/
void updateCRSCali(bool isConnected)
{
  ...
  if (nh.connected())
  {
    if (isEnded == false)
    {
      sprintf(log_msg, "Start Calibration of CRS Gyros");
      nh.loginfo(log_msg);
      ...
      while (i < numSamples)
      {
        ...
        if (currentMillis - previousMillis >= interval) {
          // save the last time sample was captured
          previousMillis = currentMillis;


          sumCRS03 += analogRead(CRS03_PIN);
          sumCRS09 += analogRead(CRS09_PIN);

          ++i;
        }
      }
      sprintf(log_msg, "Calibration End");
      CRS03_zero = (double)sumCRS03 / (double)numSamples;
      CRS09_zero = (double)sumCRS09 / (double)numSamples;
